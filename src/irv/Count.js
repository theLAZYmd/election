...
class Data {
	...
	public eliminated: string[]
	public eliminate: string[]
	public finished: boolean
	public votes: any[]

	constructor(candidates: string[], votes: any[], ties?: string[], public cycle = 0) {
...
export default class Main {
	...
	constructor(private candidates: string[], private votes: any[]) {}

	public rank() {
		let data = new Data(this.candidates, this.votes);
		while (!data.finished) {
			data = this.cycle(data);
		}
	}

	private cycle(data: Data): Data {
		let m = this.getMap(data.candidates, data.votes); //create a new map from {  candidates => no. of first preference votes for candidate }
		let removeList;
	...
	private getMap(votes: string[], candidates: string[]) { //returns a map based on first preference votes
	...
	static rank_deprecated(candidates, votes) {
		try {
